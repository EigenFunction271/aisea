...
const fragmentShader = `
  ...
  // Mostly black with peach and lavender accents
  vec3 black = vec3(0.0, 0.0, 0.0); // #000000
  vec3 subtlePeach = vec3(0.15, 0.08, 0.06); // Very dark peach
  vec3 peach = vec3(1.0, 0.71, 0.65); // #FFB5A7 for highlights
  ...
  void main() {
    ...
    // Add peach highlights where the neural network is more intense (doubled)
    float peachIntensity = smoothstep(0.3, 0.7, intensity);
    vec3 peachAccent = peach * peachIntensity * 0.6; // Doubled to 60% opacity
    
    // Add lavender highlights using different channel for variation
    float lavenderIntensity = smoothstep(0.2, 0.6, cppn.g);
    vec3 lavenderAccent = lavender * lavenderIntensity * 0.6; // 60% opacity
    
    vec3 finalColor = baseColor + peachAccent + lavenderAccent;
    
    // Keep it mostly black but allow more brightness for accents
    finalColor = clamp(finalColor, vec3(0.0), vec3(0.5));
