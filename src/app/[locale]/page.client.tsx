...
const WorldMap = dynamic(
  () => import("@/components/ui/map").then((mod) => ({ default: mod.WorldMap })),
  {
    ssr: false,
    loading: () => (
      <div className="w-full h-[400px] bg-black/20 animate-pulse rounded-lg" />
    ),
  },
);
    ...
    <>
      ...
      <main className="w-full min-h-screen">
        ...
        <section
          ...
          className="fixed inset-0 w-screen h-screen bg-black z-10 pointer-events-none"
        >
          <ShaderBackground />
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center space-y-4 px-4 pointer-events-auto z-10">
            <BlurFade delay={0} duration={0.8} yOffset={20}>
              <TextShimmer
                as="h1"
                className="font-[family-name:var(--font-perfectly-nineties)] text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-bold leading-tight [--base-color:#e4e4e7] [--bg:linear-gradient(90deg,#0000_calc(50%-var(--spread)),#60a5fa,#a78bfa,#f472b6,#fbbf24,#ffffff,#0000_calc(50%+var(--spread)))]"
                duration={1}
                spread={20}
                once
                delay={1.75}
            ...
            <BlurFade delay={0.5}>
              <div className="flex items-center justify-center gap-4 mt-2 relative z-20">
                ...
                >
                  <Link href="/work-with-us">{t("hero.workWithUs")}</Link>
                </Button>
              </div>
            </BlurFade>
        ...
        <div className="relative z-20 flex justify-center gap-4 py-8 bg-black">
          ...
          <Button
            ...
            className="font-[family-name:var(--font-geist-mono)] bg-transparent border-white/20 text-white hover:bg-white/10 hover:border-white/40 font-medium rounded-full"
          >
            <Link href="/work-with-us">{t("nav.workWithUs")}</Link>
          </Button>
        </div>
